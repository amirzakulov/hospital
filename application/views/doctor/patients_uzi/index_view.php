<?php $this->load->view('doctor/patients_uzi/header_template_view'); ?>
<div class="row">
    <div class="col-md-12">
        <table class="table table-border table-striped custom-table datatable_order_desc mb-0 compact">
            ...
            <tr>
                ...
                <th class="align-text-top" width="10%"><?= lang("index_dob_th"); ?></th>
                ...
                <th class="align-text-top" width="1%">action</th>
            </tr>
            </thead>
            <tbody>
            <?php foreach ($incomplete_patients as $i_patient) {?>
                <tr>
                    ...
                    <td>
                        <a href="<?= site_url("doctor/patients_uzi/patient2/".$i_patient["id"]) ?>"><?= $i_patient["last_name"] ." ". $i_patient["first_name"] ." ". $i_patient["surname"];?></a>
                    ...
                    <td class="d-none font-weight-bold text-danger js_show_payment_items show_payment_items" data-id="<?= $i_patient["patient_id"]; ?>" data-payment-id="<?= $i_patient["id"]; ?>">
                    ...
                    <td><?php switch ($i_patient["uzi_status"]) {
                            case 1:
					...
					<td class="text-right">
						<div class="dropdown dropdown-action">
							...
							<div class="dropdown-menu dropdown-menu-right">
								<a class="dropdown-item pt-3 pb-2" href="<?= site_url("doctor/patients_uzi/patient_edit/".$i_patient["patient_id"]); ?>"><i class="fa fa-pencil m-r-5"></i> <?= lang("general_edit"); ?></a>
							</div>
						</div>
					</td>
                </tr>
	...
	<div class="col-md-12">
		<table class="table table-border table-striped custom-table datatable_order_desc mb-0 compact">
			...
			<?php foreach ($completed_patients as $c_patient) {?>
				<?php if($c_patient["uzi_status"] > 0) { ?>
					<tr>
						<td>
							<strong><?= $c_patient["id"];?></strong><br>
							<span><?= $c_patient["created_date"];?></span>
						</td>
						<td class="font-weight-bold"><?= strtoupper($c_patient["username"]); ?></td>
						<td>
							<a href="<?= site_url("doctor/patients_uzi/patient_info/".$c_patient["id"]) ?>"><?= $c_patient["last_name"] ." ". $c_patient["first_name"] ." ". $c_patient["surname"];?></a>
							<div class="doc-prof doc-prof--mb0">
						...
						<td class="d-none font-weight-bold text-danger js_show_completed_payment_items show_payment_items" data-id="<?= $c_patient["patient_id"]; ?>" data-payment-id="<?= $c_patient["id"]; ?>">
...
<?php $this->load->view('doctor/patients_uzi/footer_template_view'); ?>
