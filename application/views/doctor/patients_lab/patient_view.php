...
//	echo "</pre>";
	...
	$default_value = is_null($laboratory["result"]) ? $laboratory["default_value"]:$laboratory["result"];
				...
				<div class="lab_result_input_block mb-1">
					<div class="input-group">
						...
						<input type="text" name="result[]" class="form-control js_tabInput" placeholder="Норма: '.($laboratory["norma"]).'"
							   value="'.($default_value).'" id="'.($laboratory["id"]).'" data-precategory="'.($laboratory["parent_id"]).'" data-is-parent-id="'.($is_parent_id).'" '.(in_array($patient["laboratory_status"], array(1)) ? "disabled='disabled'":"").'
