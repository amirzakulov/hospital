<?php $this->load->view('admin/registry/header_template_view'); ?>
    ...
    <div class="row">
        <div class="col-md-12">
            <table class="table table-border table-striped custom-table datatable_patients mb-0 compact" id="incomplete_patients">
                ...
                <?php foreach ($incomplete_patients as $i_patient) {?>
                    <tr id="js_row_<?= $i_patient["payment_id"] ?>">
                        ...
                        <td class="font-weight-bold text-danger js_show_payment_items show_payment_items" data-id="<?= $i_patient["id"]; ?>" data-payment-id="<?= $i_patient["payment_id"]; ?>" data-url="<?= site_url("admin/registry/ajax_selected_items"); ?>">
                            <span class="js_payments_<?= $i_patient["payment_id"]; ?>"><?= $i_patient["paid"] ."/".$i_patient["total"]; ?></span>
                            <div class="doc-prof doc-prof--mb0">
                                <?= date("d M Y", strtotime($i_patient["created_date"])); ?> <?= ($i_patient["debt"] > 0) ? '<span class="badge badge-danger">Қарз</span>':''; ?>
                        ...
                        <td>
                            <?php if($i_patient["service_status"] == 1) {?>
                                <button class="btn btn-primary js_complete_service" title="Тамомлаш" id="qabul_tamom" data-payment-id="<?= $i_patient["payment_id"]; ?>" data-url="<?= site_url("admin/registry/ajax_patient_service_status"); ?>">Беркитиш</button>
                            <?php } else { ?>
                                <span class="badge badge-pill js_order_status <?= $i_patient["order_status"] == 0 ? "badge-info-border":"badge-success-border"; ?> p-2">
                                <?php
                        ...
                        <td class="text-right">
                            <div class="dropdown dropdown-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a class="dropdown-item" href="<?= site_url("admin/registry/add_items/".$i_patient["id"]); ?>"><i class="fa fa-plus-circle m-r-5 text-primary"></i> Янги тўлов</a>
                                    ...
                                    <a class="dropdown-item" href="<?= site_url("admin/registry/edit/".$i_patient["id"]); ?>"><i class="fa fa-pencil m-r-5 "></i> <?= lang("general_edit"); ?></a>
                                </div>
                            </div>
                        </td>
                    </tr>
                ...
                <?php foreach ($completed_patients as $c_patient) {?>
                    <tr>
                        ...
                        <td class="font-weight-bold text-danger js_show_completed_payment_items show_payment_items" data-id="<?= $c_patient["id"]; ?>" data-payment-id="<?= $c_patient["payment_id"]; ?>" data-url="<?= site_url("admin/registry/ajax_selected_items"); ?>">
                            ...
                            <div class="doc-prof doc-prof--mb0">
                                <?= date("d M Y", strtotime($c_patient["created_date"])); ?> <?= ($c_patient["debt"] > 0) ? '<span class="badge badge-danger">Қарз</span>':''; ?>
								<?= ($c_patient["discount_value"] > 0) ? '<span class="badge badge-info">Чегирма</span>':''; ?>
                                <?php $paid_services = ""; ?>
                                <?php $paid_services .= (!$c_patient["doctor_status"] ? "":"<span class='text-danger'>D</span>"); ?>
                                <?php $paid_services .= !$c_patient["laboratory_status"] ? "":" <span class='text-danger'>L</span>"; ?>
                                <?php $paid_services .= !$c_patient["uzi_status"] ? "":" <span class='text-danger'>U</span>"; ?>
                                <?php $paid_services .= !$c_patient["service_status"] ? "":" <span class='text-danger'>X</span>"; ?>
                                <div class="text-dark"><?= $paid_services; ?></div>
                            </div>
                        </td>
                        <td><?= $c_patient["partner_last_name"] ." ". $c_patient["partner_first_name"];?></td>
                        <td><span class="badge badge-pill badge-danger-border p-2 js_order_status">Ёпилган</span></td>
                        <td class="text-right">
                            <div class="dropdown dropdown-action">
                                <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
                                <div class="dropdown-menu dropdown-menu-right">
